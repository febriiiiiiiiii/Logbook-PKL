...
@section('content')
    <div class="mt-14 p-4 sm:ml-64">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white p-6">
            <form action="{{ route('pembimbing_lapangan.update', $pembimbingLapangan->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div>
                    <label for="nama" class="block text-gray-700 text-sm font-semibold mb-2">Nama Pembimbing Lapangan:</label>
